...
namespace MovieRenamer
{
	...
	public class Movie
	{
		...
		public bool needRenaming()
		{
			...
			{
				original = original.Replace(fi.Extension, "");
			}
		...
		{
			String renameFormat = "<moviename> (<year> <disk> <lang>)";
			String appFolder = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData)+@"\MovieRenamer\";
			if (File.Exists(appFolder+"format.dat"))
			{
				StreamReader strm = new StreamReader( appFolder+"format.dat");
				if (strm != null)
				{
					renameFormat = strm.ReadLine();
					strm.Close();
				}
			}
